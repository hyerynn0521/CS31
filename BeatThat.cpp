...
namespace cs31
{
    ...
    // TODO: called to allow the human player to randomly roll its dice V
    void BeatThat::humanPlay( )
    {
        mHuman.roll();
    }
    
    // TODO: called to force a certain human player roll V
    void BeatThat::humanPlay( Die d1, Die d2 )
    {
        mHuman.roll(d1, d2);
    }

    // TODO: called to allow the computer player to randomly roll its dice V
    void BeatThat::computerPlay( )
    {
        mComputer.roll();
    }

    // TODO: called to force a certain computer player roll V
    void BeatThat::computerPlay( Die d1, Die d2 )
    {
        mComputer.roll(d1, d2);
    }
        
    // TODO: called by isGameOver( ) to figure out the current state of the game V
    ...
    BeatThat::GAMEOUTCOME  BeatThat::determineGameOutcome( ) const
    {
        ...
        if(getBoard().getTurnsLeft() > 0){
            result = GAMEOUTCOME::GAMENOTOVER;
        }
        else if(mBoard.didHumanWin() == true && mBoard.didComputerWin() == false){
            result = GAMEOUTCOME::HUMANWONGAME;
        }
        else if(mBoard.didComputerWin() == true && mBoard.didHumanWin() == false ){
            result = GAMEOUTCOME::COMPUTERWONGAME;
        }
        else if(mBoard.didHumanWin() == false && mBoard.didComputerWin() == false && getBoard().getTurnsLeft() == 0){
